    ...
    ({ title, ingredients, hashTable }, ref) => {
      return (
        <section className="mb-10" ref={ref}>
          ...
          <div
            className={`pt-6 pl-4 pr-4 ${sectionStyles["burger-ingredients-container"]}`}
          >
            {ingredients.map((ingredient) => (
              <IngredientCard key={ingredient._id} ingredient={ingredient} count={hashTable[ingredient._id]} />
            ))}
          </div>
        </section>
      );
